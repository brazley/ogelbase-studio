-- ============================================
-- Migration 006: Enable RLS with Permissive Policies (PRODUCTION VERSION)
-- ============================================
...
--
-- Strategy:
--   1. Pre-flight checks (PostgreSQL version, table existence)
--   2. Enable RLS on all platform schema tables
--   3. Create permissive policies (allow all operations)
--   4. Verify no existing queries are blocked
--
...
BEGIN
    ...
    IF current_setting('server_version_num')::int < 90500 THEN
        RAISE EXCEPTION 'PostgreSQL 9.5 or higher is required for Row Level Security (current version: %)',
            current_setting('server_version');
    END IF;

    -- Verify critical tables exist (from migrations 001-005)
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'platform'
        AND table_name = 'organizations'
    ) THEN
        RAISE EXCEPTION 'Migration 001 must be applied first (platform.organizations table missing)';
    END IF;
    ...
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION 'Migration 003 must be applied first (platform.users table missing)';
    END IF;

    RAISE NOTICE 'Pre-flight checks passed - PostgreSQL version: %, platform schema exists',
        current_setting('server_version');
END $$;
...
RAISE NOTICE 'Phase 1 complete: RLS enabled on 24 tables';
...
RAISE NOTICE 'Phase 2 complete: 24 permissive policies created';
...
BEGIN
    ...
    SELECT
        'rls_enabled'::TEXT,
        (SELECT COUNT(*) FROM pg_tables WHERE schemaname = 'platform' AND rowsecurity = true) >= 24,
        'RLS is enabled on 24+ tables'::TEXT;

    -- Test 5: Policies exist
    RETURN QUERY
    SELECT
        'policies_exist'::TEXT,
        (SELECT COUNT(*) FROM pg_policies WHERE schemaname = 'platform') >= 24,
        '24+ RLS policies are defined'::TEXT;
END;
$$ LANGUAGE plpgsql;
...
--   ~2-3% query overhead (RLS check runs but policy always passes)
--
-- Next Steps:
--   1. Monitor application for 24-48 hours
--   2. Verify no "permission denied" errors
--   3. Check query performance (should be <5% overhead)
--   4. When stable, prepare application code for Migration 007
--
-- Rollback:
