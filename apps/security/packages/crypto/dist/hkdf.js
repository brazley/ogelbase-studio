...
async function hmacSha256(key, data) {
    const cryptoKey = await crypto.subtle.importKey('raw', key, { name: 'HMAC', hash: HASH_ALGORITHM }, false, ['sign']);
...
export async function hkdfExpand(prk, info, length) {
    ...
    if (length > MAX_OUTPUT_LENGTH) {
        throw new Error(`HKDF: output length too long (max ${MAX_OUTPUT_LENGTH} bytes for SHA-256)`);
    }
    ...
    const blocks = [];
    let previousBlock = new Uint8Array(0); // T(0) = empty string
    for (let i = 1; i <= n; i++) {
        // T(i) = HMAC-Hash(PRK, T(i-1) | info | i)
        const input = concat(previousBlock, info, new Uint8Array([i]) // Counter as single byte
        );
