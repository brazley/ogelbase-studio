 ...
 * - Error handling
 */
import { encrypt, decrypt, generateKey, generateNonce, encryptString, decryptString, AESGCMError } from '../aes-gcm';
describe('AES-256-GCM', () => {
    ...
    describe('Nonce Generation', () => {
        ...
        test('generateNonce() produces cryptographically random nonces', () => {
            ...
            const uniqueNonces = new Set(nonces.map(n => Buffer.from(n).toString('hex')));
            expect(uniqueNonces.size).toBe(1000);
            // Check that nonces have sufficient entropy
            for (const nonce of nonces) {
                const allZero = nonce.every(byte => byte === 0);
                const allOne = nonce.every(byte => byte === 0xff);
                expect(allZero).toBe(false);
                expect(allOne).toBe(false);
            }
        });
    ...
    describe('Round-Trip Encryption/Decryption', () => {
        ...
        test('same plaintext produces different ciphertexts (randomized nonce)', async () => {
            ...
            expect(Buffer.from(encrypted1.ciphertext).equals(Buffer.from(encrypted2.ciphertext))).toBe(false);
        });
    });
    ...
    describe('NIST SP 800-38D Test Vectors', () => {
        ...
        test('NIST Test Case 13: 256-bit key, empty plaintext', async () => {
            ...
            const result = await crypto.subtle.encrypt({ name: 'AES-GCM', iv: nonce, tagLength: 128 }, cryptoKey, plaintext);
        ...
        test('NIST Test Case 14: 256-bit key, 128-bit plaintext', async () => {
            ...
            expect(bytesToHex(tag)).toBe(expectedTagHex);
            // Verify decryption
            const decrypted = await crypto.subtle.decrypt({ name: 'AES-GCM', iv: nonce, tagLength: 128 }, cryptoKey, result);
            expect(bytesToHex(new Uint8Array(decrypted))).toBe(plaintextHex);
        });
        test('NIST Test Case 15: 256-bit key, 256-bit plaintext', async () => {
            const keyHex = 'feffe9928665731c6d6a8f9467308308feffe9928665731c6d6a8f9467308308';
            const ivHex = 'cafebabefacedbaddecaf888';
            const plaintextHex = 'd9313225f88406e5a55909c5aff5269a86a7a9531534f7da2e4c303d8a318a721c3c0c95956809532fcf0e2449a6b525b16aedf5aa0de657ba637b39';
            const expectedCiphertextHex = '522dc1f099567d07f47f37a32a84427d643a8cdcbfe5c0c97598a2bd2555d1aa8cb08e48590dbb3da7b08b1056828838c5f61e6393ba7a0abcc9f662';
            const expectedTagHex = '76fc6ece0f4e1768cddf8853bb2d551b';
            const key = hexToBytes(keyHex);
            const nonce = hexToBytes(ivHex);
            const plaintext = hexToBytes(plaintextHex);
            const cryptoKey = await crypto.subtle.importKey('raw', key, { name: 'AES-GCM', length: 256 }, false, ['encrypt', 'decrypt']);
        ...
        test('NIST Test Case with AAD: 256-bit key with additional data', async () => {
            const keyHex = 'feffe9928665731c6d6a8f9467308308feffe9928665731c6d6a8f9467308308';
            const ivHex = 'cafebabefacedbaddecaf888';
            const plaintextHex = 'd9313225f88406e5a55909c5aff5269a86a7a9531534f7da2e4c303d8a318a721c3c0c95956809532fcf0e2449a6b525b16aedf5aa0de657ba637b39';
            const aadHex = 'feedfacedeadbeeffeedfacedeadbeefabaddad2';
            const expectedCiphertextHex = '522dc1f099567d07f47f37a32a84427d643a8cdcbfe5c0c97598a2bd2555d1aa8cb08e48590dbb3da7b08b1056828838c5f61e6393ba7a0abcc9f662';
            const expectedTagHex = '3612d2e79e3b0785561be14aaca2fccb';
            const key = hexToBytes(keyHex);
            const nonce = hexToBytes(ivHex);
            const plaintext = hexToBytes(plaintextHex);
            const aad = hexToBytes(aadHex);
            const cryptoKey = await crypto.subtle.importKey('raw', key, { name: 'AES-GCM', length: 256 }, false, ['encrypt', 'decrypt']);
            const result = await crypto.subtle.encrypt({ name: 'AES-GCM', iv: nonce, additionalData: aad, tagLength: 128 }, cryptoKey, plaintext);
            const resultArray = new Uint8Array(result);
            const ciphertext = resultArray.slice(0, -16);
            const tag = resultArray.slice(-16);
            expect(bytesToHex(ciphertext)).toBe(expectedCiphertextHex);
...
//# sourceMappingURL=aes-gcm.test.js.map