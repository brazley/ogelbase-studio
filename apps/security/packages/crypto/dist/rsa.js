...
/**
 * PSS padding parameters
 */
const PSS_PARAMS = {
    name: 'RSA-PSS',
    saltLength: 32, // 256 bits (matches SHA-256 output)
};
// ============================================================================
// Key Generation
// ============================================================================
...
export async function generateKeyPair() {
    try {
        const keyPair = await crypto.subtle.generateKey(RSA_ALGORITHM, true, // extractable: can export keys for storage
        ['sign', 'verify']);
        return {
        ...
        throw new RSAError(`Key generation failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
// ============================================================================
// Signing
...
export async function sign(data, privateKey) {
    try {
        const signature = await crypto.subtle.sign(PSS_PARAMS, privateKey, data);
        return new Uint8Array(signature);
    }
    catch (error) {
        throw new RSAError(`Signing failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
// ============================================================================
// Verification
...
export async function verify(data, signature, publicKey) {
    try {
        return await crypto.subtle.verify(PSS_PARAMS, publicKey, signature, data);
    }
    catch (error) {
        // Verification errors mean invalid signature (return false, not throw)
        return false;
    }
}
// ============================================================================
...
export async function exportKeyPair(keyPair) {
    try {
        ...
        throw new RSAError(`Key export failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
/**
...
export async function exportPublicKey(publicKey) {
    try {
        ...
        throw new RSAError(`Public key export failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
/**
...
export async function exportPrivateKey(privateKey) {
    try {
        ...
        throw new RSAError(`Private key export failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
/**
...
export async function importPublicKey(publicKeyBytes) {
    try {
        return await crypto.subtle.importKey('spki', publicKeyBytes, {
            name: 'RSA-PSS',
            hash: 'SHA-256',
        }, true, // extractable (can re-export if needed)
        ['verify']);
    }
    catch (error) {
        throw new RSAError(`Public key import failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
/**
...
export async function importPrivateKey(privateKeyBytes) {
    try {
        return await crypto.subtle.importKey('pkcs8', privateKeyBytes, {
            name: 'RSA-PSS',
            hash: 'SHA-256',
        }, true, // extractable (can re-export if needed)
        ['sign']);
        ...
        throw new RSAError(`Private key import failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
// ============================================================================
// Utility Functions
 ...
 */
export function getModulusLength(key) {
 ...
 */
export async function verifyKeyPairMatch(keyPair) {
    try {
        const testData = new Uint8Array(32);
        crypto.getRandomValues(testData);
        const signature = await sign(testData, keyPair.privateKey);
        return await verify(testData, signature, keyPair.publicKey);
    }
    catch {
        return false;
    }
