...
export async function generateKeyPair(): Promise<RSAKeyPair> {
  try {
    const keyPair = await crypto.subtle.generateKey(
      RSA_ALGORITHM,
      true, // extractable: can export keys for storage
      ['sign', 'verify']
    );
...
export async function importPublicKey(
  ...
  try {
    ...
    throw new RSAError(
      `Public key import failed: ${error instanceof Error ? error.message : 'Unknown error'}`
    );
  }
}
...
export async function importPrivateKey(
  ...
  try {
    ...
    throw new RSAError(
      `Private key import failed: ${error instanceof Error ? error.message : 'Unknown error'}`
    );
  }
}
 ...
 */
export function getModulusLength(key: CryptoKey): number {
  const algorithm = key.algorithm as RsaHashedKeyGenParams;
 ...
 */
export async function verifyKeyPairMatch(
  keyPair: RSAKeyPair
): Promise<boolean> {
  try {
