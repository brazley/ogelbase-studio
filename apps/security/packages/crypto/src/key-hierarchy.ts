 ...
 * ```
 */
export async function deriveDeviceMasterKey(
  umk: UserMasterKey,
  deviceId: string
): Promise<DeviceMasterKey> {
  // Validate inputs
  if (!umk?.key || umk.key.length !== KEY_LENGTH) {
    throw new KeyHierarchyError(
      `Invalid UMK: expected ${KEY_LENGTH} bytes, got ${umk?.key?.length ?? 0} bytes`
    );
  }
  ...
  try {
    ...
    throw new KeyHierarchyError(
      'Failed to derive Device Master Key',
      error as Error
    );
  }
}
 ...
 * ```
 */
export async function deriveDeviceKeys(dmk: DeviceMasterKey): Promise<DeviceKeys> {
  // Validate DMK
  if (!dmk?.key || dmk.key.length !== KEY_LENGTH) {
    throw new KeyHierarchyError(
      `Invalid DMK: expected ${KEY_LENGTH} bytes, got ${dmk?.key?.length ?? 0} bytes`
    );
  }
  ...
  try {
    ...
    const metadataEncryptionKey = await hkdf(
      mekSalt,
      dmk.key,
      mekContext,
      KEY_LENGTH
    ...
    throw new KeyHierarchyError('Failed to derive device keys', error as Error);
  }
}
 ...
 * ```
 */
export async function deriveKeysFromUMK(
  umk: UserMasterKey,
  deviceId: string
): Promise<{
  dmk: DeviceMasterKey;
