...
function validateKey(key: Uint8Array): void {
  if (key.length !== AES_KEY_LENGTH) {
    throw new AESGCMError(
      `Invalid key length: expected ${AES_KEY_LENGTH} bytes, got ${key.length} bytes`
    );
  }
}
...
function validateNonce(nonce: Uint8Array): void {
  if (nonce.length !== NONCE_LENGTH) {
    throw new AESGCMError(
      `Invalid nonce length: expected ${NONCE_LENGTH} bytes, got ${nonce.length} bytes`
    );
  }
}
...
async function importKey(key: Uint8Array): Promise<CryptoKey> {
  ...
  try {
    ...
    throw new AESGCMError('Failed to import key', error as Error);
  }
}
...
export async function encrypt(
  ...
  try {
    ...
    throw new AESGCMError('Encryption failed', error as Error);
  }
}
    ...
    const decrypted = await crypto.subtle.decrypt(
      {
        ...
        additionalData: encrypted.additionalData as BufferSource | undefined,
        tagLength: TAG_LENGTH * 8 // 128 bits
      },
      cryptoKey,
...
export async function decryptString(
  ...
  try {
    ...
    throw new AESGCMError('Decrypted data is not valid UTF-8', error as Error);
  }
}
